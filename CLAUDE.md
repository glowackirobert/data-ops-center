...
- It runs 1 iteration of 500,000,000 messages across 2 threads, and runs to that count rather than to a clock — `shutdownExecutor` waits without a deadline, so the run is bounded by `NUMBER_OF_MESSAGES` alone (both constants live in `KafkaCustomTopicProducer.java`). Sizing is a **disk** decision, not a time one: a trade row measures ~34 bytes in Pinot, so 500M is ~17 GB there plus the topic's own 10 GB retention cap — check free space before raising it.
