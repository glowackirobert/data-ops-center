...
- **Analytics** — a native "overview" strip (Total rows tile + last-24h network rush-hour bar chart, fetched straight from Pinot via `/api/stats` and `/api/network-hourly`, painting in tens of ms) above the Superset dashboard embedded via the Superset Embedded SDK. The overview charts P1/P7 were moved out of Superset deliberately: the embedded iframe spends seconds booting Superset's frontend bundle before any chart data loads, which buried two always-on-screen numbers behind the slowest part of the page. The filter-free "Overview" Superset dashboard (`Gdansk_Public_Transport_Overview.yaml`) still exists and imports, but is no longer embedded by the web app.
